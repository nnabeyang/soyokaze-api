...
extension comatprototypes {
    ...
    public indirect enum ServerRevokeAppPassword_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func ServerRevokeAppPassword(input: comatprototypes.ServerRevokeAppPassword_Input) async throws -> Bool {
        let params: Bool? = nil
        do {
            return try await fetch(endpoint: "com.atproto.server.revokeAppPassword", contentType: "application/json", httpMethod: .post, params: params, input: input, retry: true)
        } catch let error as UnExpectedError {
            throw comatprototypes.ServerRevokeAppPassword_Error(error: error)
        }
    }
}
