...
extension comatprototypes {
    ...
    public indirect enum IdentityResolveHandle_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func IdentityResolveHandle(handle: String) async throws -> comatprototypes.IdentityResolveHandle_Output {
        let params: Parameters = ["handle": .string(handle)]
        do {
            return try await fetch(endpoint: "com.atproto.identity.resolveHandle", contentType: "*/*", httpMethod: .get, params: params, input: Bool?.none, retry: true)
        } catch let error as UnExpectedError {
            throw comatprototypes.IdentityResolveHandle_Error(error: error)
        }
    }
}
