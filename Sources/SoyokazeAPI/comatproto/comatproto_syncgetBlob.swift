...
extension comatprototypes {
    public indirect enum SyncGetBlob_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func SyncGetBlob(cid: String, did: String) async throws -> Data {
        let params: Parameters = ["cid": .string(cid), "did": .string(did)]
        do {
            return try await fetch(endpoint: "com.atproto.sync.getBlob", contentType: "*/*", httpMethod: .get, params: params, input: Bool?.none, retry: true)
        } catch let error as UnExpectedError {
            throw comatprototypes.SyncGetBlob_Error(error: error)
        }
    }
}
