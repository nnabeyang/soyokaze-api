...
extension comatprototypes {
    ...
    public indirect enum IdentitySubmitPlcOperation_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func IdentitySubmitPlcOperation(input: comatprototypes.IdentitySubmitPlcOperation_Input) async throws -> Bool {
        let params: Bool? = nil
        do {
            return try await fetch(endpoint: "com.atproto.identity.submitPlcOperation", contentType: "application/json", httpMethod: .post, params: params, input: input, retry: true)
        } catch let error as UnExpectedError {
            throw comatprototypes.IdentitySubmitPlcOperation_Error(error: error)
        }
    }
}
