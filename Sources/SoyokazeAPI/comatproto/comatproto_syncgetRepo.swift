...
extension comatprototypes {
    public indirect enum SyncGetRepo_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func SyncGetRepo(did: String, since: String? = nil) async throws -> Data {
        let params: Parameters = ["did": .string(did), "since": .string(since)]
        do {
            return try await fetch(endpoint: "com.atproto.sync.getRepo", contentType: "*/*", httpMethod: .get, params: params, input: Bool?.none, retry: true)
        } catch let error as UnExpectedError {
            throw comatprototypes.SyncGetRepo_Error(error: error)
        }
    }
}
