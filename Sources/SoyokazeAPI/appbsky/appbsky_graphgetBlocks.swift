...
extension appbskytypes {
    ...
    public indirect enum GraphGetBlocks_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func GraphGetBlocks(cursor: String? = nil, limit: Int? = nil) async throws -> appbskytypes.GraphGetBlocks_Output {
        let params: Parameters = ["cursor": .string(cursor), "limit": .integer(limit)]
        do {
            return try await fetch(endpoint: "app.bsky.graph.getBlocks", contentType: "*/*", httpMethod: .get, params: params, input: Bool?.none, retry: true)
        } catch let error as UnExpectedError {
            throw appbskytypes.GraphGetBlocks_Error(error: error)
        }
    }
}
