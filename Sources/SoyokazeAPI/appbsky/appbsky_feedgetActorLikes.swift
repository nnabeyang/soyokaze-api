...
extension appbskytypes {
    public struct FeedGetActorLikes_Output: Codable, Sendable {
    ...
    public indirect enum FeedGetActorLikes_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func FeedGetActorLikes(actor: String, cursor: String? = nil, limit: Int? = nil) async throws -> appbskytypes.FeedGetActorLikes_Output {
        let params: Parameters = ["actor": .string(actor), "cursor": .string(cursor), "limit": .integer(limit)]
        do {
            return try await fetch(endpoint: "app.bsky.feed.getActorLikes", contentType: "*/*", httpMethod: .get, params: params, input: Bool?.none, retry: true)
        } catch let error as UnExpectedError {
            throw appbskytypes.FeedGetActorLikes_Error(error: error)
        }
    }
}
