...
extension appbskytypes {
    public struct FeedGetFeed_Output: Codable, Sendable {
    ...
    public indirect enum FeedGetFeed_Error: XRPCError {
...
extension XRPCClientProtocol {
    public mutating func FeedGetFeed(cursor: String? = nil, feed: String, limit: Int? = nil) async throws -> appbskytypes.FeedGetFeed_Output {
        let params: Parameters = ["cursor": .string(cursor), "feed": .string(feed), "limit": .integer(limit)]
        do {
            return try await fetch(endpoint: "app.bsky.feed.getFeed", contentType: "*/*", httpMethod: .get, params: params, input: Bool?.none, retry: true)
        } catch let error as UnExpectedError {
            throw appbskytypes.FeedGetFeed_Error(error: error)
        }
    }
}
